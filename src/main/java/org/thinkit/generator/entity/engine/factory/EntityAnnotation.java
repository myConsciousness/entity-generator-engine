...
/**
 * エンティティクラスのアノテーションを生成する処理を定義したファクトリークラスです。
 *
 * @author Kato Shinya
 * @since 1.0.0
 */
...
public final class EntityAnnotation extends Annotation {
     ...
     */
    private EntityAnnotation(@NonNull AnnotationPattern annotationPattern) {
        super(annotationPattern);
    }

    /**
     * 引数として渡された情報を基に {@link EntityAnnotation} クラスの新しいインスタンスを生成し返却します。
     *
     * @param annotationPattern アノテーションパターン
     * @return {@link EntityAnnotation} クラスの新しいインスタンス
     *
     * @exception NullPointerException 引数として {@code null} が渡された場合
     */
    protected static Annotation of(@NonNull AnnotationPattern annotationPattern) {
        return new EntityAnnotation(annotationPattern);
    }
